...
def build_training_data(training_schema: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[~df["weight_class"].str.contains("women", case=False, na=False)].copy()
...
def main() -> None:
    base_data_dir = Path("data") / "current_model"
    base_features_dir = base_data_dir / "features"
    features_dir = Path("data") / "WIP 0.0.3" / "features"
    features_dir.mkdir(parents=True, exist_ok=True)

    training_schema = load_csv(base_features_dir / "ml_training_schema.csv")
    fights_clean = load_csv(base_data_dir / "clean" / "fights_clean.csv")
    ...
    filtered_schema = training_schema.copy()
    filtered_schema["weight_class"] = filtered_schema["weight_class"].fillna("Unknown")
    filtered_schema = filtered_schema[
        ~filtered_schema["weight_class"].str.contains("women", case=False, na=False)
    ].copy()
    ...
    save_csv(filtered_schema, features_dir / "ml_training_schema.csv")
    ...
    fighter_snapshots = load_csv(base_features_dir / "fighter_snapshots.csv")
    if not fighter_snapshots.empty:
        fighter_snapshots["weight_class"] = fighter_snapshots["weight_class"].fillna("Unknown")
        fighter_snapshots = fighter_snapshots[
            ~fighter_snapshots["weight_class"].str.contains("women", case=False, na=False)
        ].copy()
    save_csv(fighter_snapshots, features_dir / "fighter_snapshots.csv")

    with sqlite3.connect(str(db_path)) as conn:
        filtered_schema.to_sql("ml_training_schema_0_0_3", conn, if_exists="replace", index=False)
        training_data.to_sql("ml_training_data_0_0_3", conn, if_exists="replace", index=False)
        split_frames.get("train", pd.DataFrame(columns=training_data.columns)).to_sql("ml_training_data_train_0_0_3", conn, if_exists="replace", index=False)
        split_frames.get("validation", pd.DataFrame(columns=training_data.columns)).to_sql("ml_training_data_validation_0_0_3", conn, if_exists="replace", index=False)
        split_frames.get("test", pd.DataFrame(columns=training_data.columns)).to_sql("ml_training_data_test_0_0_3", conn, if_exists="replace", index=False)
        fighter_snapshots.to_sql("fighter_snapshots_0_0_3", conn, if_exists="replace", index=False)
